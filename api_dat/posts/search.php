...
require_once '../../source/models/PostModel.php'; // Đảm bảo đã bao gồm UserModel
...
// Tìm kiếm người dùng
$results = $postModel->searchPost($keyword, $offset, $limit);

// Kiểm tra kết quả và trả về phản hồi JSON
if (!empty($results)) {
    echo json_encode($results); // Nếu tìm thấy người dùng
} else {
    echo json_encode(["message" => "No post found."]); // Nếu không tìm thấy người dùng
}
?>
